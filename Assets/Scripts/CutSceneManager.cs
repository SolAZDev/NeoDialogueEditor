...
public class CutSceneManager : MonoBehaviour {
	public enum TextScroll {
		...
		public TextScroll TextScrollType;
		...
		protected Vector3[] MoveList, RotateList, ScaleList;
		protected float[] MoveRotSpeed;

		protected bool fade, SceneIn, flash, UpGUI, canUpdate = false;
		public delegate void Cnt ();
	...
	public void Update () {
		if (canUpdate) {
			...
			for (int i = 0; i < Actors.Count; i++) {
				...
				if (Actors[i].transform.localScale != ScaleList[i]) {
					Actors[i].transform.localScale = Vector3.Lerp (Actors[i].transform.localScale, ScaleList[i], Time.deltaTime * MoveRotSpeed[i]);
				}
			}
			if (fade && FlashFade != null) {
				if (SceneIn) {
	...
	public IEnumerator UpdateLine (string customText = "") {
		ContinueBtn.interactable = false;
		if (ContinueIcon != null) { ContinueIcon.gameObject.SetActive (false); }
		TextBox.text = System.String.Empty;
		switch (TextScrollType) {
			case TextScroll.Fade:
				if (TextAnim != null) {
					TextAnim.SetTrigger ("Fade");
					yield return new WaitForSeconds (.5f);
					if (customText == "") { TextBox.text = ActiveChoice.dialogue; } else { TextBox.text = customText; }

					yield return null;
				}
				yield return null;
				break;
			case TextScroll.TextScroll:
				int id = ActiveChoice.id;
				if (customText == "") {
					for (int i = 0; i < ActiveChoice.dialogue.ToCharArray ().Length; i++) {
						TextBox.text = ActiveChoice.dialogue.Substring (0, i);
						yield return null;
						TextBox.text = ActiveChoice.dialogue;
					}
				} else {
					for (int i = 0; i < customText.ToCharArray ().Length; i++) {
						TextBox.text = customText.Substring (0, i);
						yield return null;
				...
				//TextBox.text = ActiveChoice.dialogue;
				break;
			case TextScroll.None:
				//TextBox.text = MainManager.Instance.Utilities.CleanString(ActiveChoice.dialogue);
				if (customText == "") { TextBox.text = ActiveChoice.dialogue; } else { TextBox.text = customText; }
				yield return new WaitForSeconds (.1f);
				break;
		}
		...
		if (ActiveLine.GetChoices ().Count > 1) {
			...
			if (ContinueIcon != null) { ContinueIcon.gameObject.SetActive (true); }
			ContinueBtn.interactable = true;
		}
		yield return null;
	}
	...
	public void ParseArguments () {
		...
		for (int i = 0; i < arguements.Length; i++) {
			...
			//Scaling an Actor
			if (arguements[i].Contains ("sc_") || arguements[i].Contains ("Scale_")) {
				argArgs = arguements[i].Split (new [] { '_' });
				if (Actors.Count > int.Parse (argArgs[1])) {
					ScaleList[int.Parse (argArgs[1])] = new Vector3 (
						(argArgs[2] != "N") ? float.Parse (argArgs[2]) : Actors[0].transform.localScale.x,
						(argArgs[3] != "N") ? float.Parse (argArgs[3]) : Actors[0].transform.localScale.y,
						(argArgs[4] != "N") ? float.Parse (argArgs[4]) : Actors[0].transform.localScale.z
					);
					MoveRotSpeed[int.Parse (argArgs[1])] = (argArgs.Length > 4) ? float.Parse (argArgs[4]) : 8f;
				}
			...
			if (arguements[i].Contains ("ShowImage_")) {
				...
				Sprite spr = Resources.Load<Sprite> ("Images/" + argArgs[1]);
				if (spr != null) {
			...
			if (arguements[i].Contains ("SetupFace_")) {
				argArgs = arguements[i].Split (new [] { '_' });
				if (Actors.Count > 0) { //Don't Get Bamboozled again!
					Sprite sprite = Resources.Load<Sprite> ("Faces/" + ((ActiveChoice.speaker != System.String.Empty) ? ActiveChoice.speaker : Actors[int.Parse (argArgs[1])].gameObject.name) + "/" + argArgs[2]);
					if (sprite == null) {
						sprite = Resources.Load<Sprite> ("Sprites/" + ((ActiveChoice.speaker != System.String.Empty) ? ActiveChoice.speaker : Actors[int.Parse (argArgs[1])].gameObject.name) + "/" + argArgs[2]);
					}
					if (sprite != null) {
						Actors[int.Parse (argArgs[1])].GetComponent<Image> ().sprite = sprite;
						Actors[int.Parse (argArgs[1])].GetComponent<Image> ().rectTransform.sizeDelta = new Vector2 (float.Parse (argArgs[3]), float.Parse (argArgs[4]));
					} else {
						print ("Sprite " + argArgs[2] + " Not found!");
					}
				} else {
					print ("Actor " + Actors[int.Parse (argArgs[1])] + " Not found!");
				}
			}
			...
			//Setup sprite
			//SetupSpirte_ID_IMG_W_H
			if (arguements[i].Contains ("ChangeSprite_")) {
				argArgs = arguements[i].Split (new [] { '_' });
				if (Actors.Count > 0) { //Don't Get Bamboozled again!
					Sprite sprite = Resources.Load<Sprite> ("Faces/" + ((ActiveChoice.speaker != System.String.Empty) ? ActiveChoice.speaker : Actors[int.Parse (argArgs[1])].gameObject.name) + "/" + argArgs[2]);
					if (sprite == null) {
					...
					if (sprite != null) {
						Actors[int.Parse (argArgs[1])].GetComponent<SpriteRenderer> ().sprite = sprite;
						//Actors[int.Parse (argArgs[1])].transform.localScale = new Vector3 (float.Parse (argArgs[3]), float.Parse (argArgs[4]), 1);
					} else {
						print ("Sprite " + argArgs[2] + " Not found!");
					}
				} else {
			...
			if (arguements[i].Contains ("ChangeSprite_")) {
				argArgs = arguements[i].Split (new [] { '_' });
				if (Actors.Count > 0) { //Don't Get Bamboozled again!
					//	print ("Sprites|Faces/" + ((ActiveChoice.speaker != System.String.Empty) ? ActiveChoice.speaker : Actors[int.Parse (argArgs[1])].gameObject.name) + "/" + argArgs[2]);
					Sprite sprite = Resources.Load<Sprite> ("Sprites/" + ((ActiveChoice.speaker != System.String.Empty) ? ActiveChoice.speaker : Actors[int.Parse (argArgs[1])].gameObject.name) + "/" + argArgs[2]);
					if (sprite == null) {
						sprite = Resources.Load<Sprite> ("Faces/" + ((ActiveChoice.speaker != System.String.Empty) ? ActiveChoice.speaker : Actors[int.Parse (argArgs[1])].gameObject.name) + "/" + argArgs[2]);
					}
					if (sprite != null) {
						Actors[int.Parse (argArgs[1])].GetComponent<SpriteRenderer> ().sprite = sprite;
					} else {
